  ...
  try {
    ...
    if (email) {
      ...
      if (!matchPassword)
        return res
          .status(400)
          .send({ status: false, message: "password is not found" });
    ...
    if (phone) {
      ...
      if (!userPhone)
        return res
          .status(400)
          .send({ status: false, message: "phone is not found" });
...
const forgotPassword = async function (req, res) {
  try {
    ...
    if (email) {
      ...
      return res
        .status(200)
        .send({ status: false, message: "success", data: userData._id });
    }
    ...
    if (phone) {
      if (!isValidPhone(phone))
        return res
          .status(400)
          .send({ status: false, message: "phone is not valid" });
      let userData = await userModel.findOne({ phone: phone });
      if (!userData)
        return res
          .status(400)
          .send({ status: false, message: " phone is not found" });
...
const resetPassword = async function (req, res) {
  try {
    ...
    if (!newPassword)
      return res
        .status(400)
        .send({ status: false, message: "please enter a new password" });
